...
"""Tests for models.lstm_ssd_mobilenet_v2_feature_extractor."""
...
from lstm_object_detection.models import lstm_ssd_mobilenet_v2_feature_extractor as feature_extractor
from object_detection.models import ssd_feature_extractor_test


class LstmSsdMobilenetV2FeatureExtractorTest(
  ...
  def _create_feature_extractor(self,
    ...
    Returns:
      An lstm_ssd_meta_arch.LSTMSSDMobileNetV2FeatureExtractor object.
    """
    min_depth = 32
    extractor = (
        feature_extractor.LSTMSSDMobileNetV2FeatureExtractor(
            is_training,
            depth_multiplier,
            min_depth,
            pad_to_multiple,
    ...
    extractor.lstm_state_depth = int(320 * depth_multiplier)
    return extractor
  ...
  def test_feature_extractor_construct_with_expected_params(self):
    ...
    extractor = (
        feature_extractor.LSTMSSDMobileNetV2FeatureExtractor(**params))
  ...
  def test_extract_features_returns_correct_shapes_256(self):
    ...
    pad_to_multiple = 1
    batch_size = 5
    expected_feature_map_shape = [(batch_size, 8, 8, 320),
                                  (batch_size, 4, 4, 256), (batch_size, 2, 2, 256),
                                  (batch_size, 1, 1, 256), (batch_size, 1, 1, 256)]
  ...
  def test_variables_only_created_in_scope(self):
    ...
    g = tf.Graph()
    with g.as_default():
      preprocessed_inputs = tf.placeholder(tf.float32, (5, 320, 320, 3))
      extractor = self._create_feature_extractor()
  ...
  def test_lstm_non_zero_state(self):
    init_state = {
        'lstm_state_c': tf.zeros([8, 8, 320]),
        'lstm_state_h': tf.zeros([8, 8, 320]),
